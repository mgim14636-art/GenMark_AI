...
| `PRODUCT_THUMBNAIL` | **완료.** 고정 화장품 목업 사진(민트톤 선물세트)에 로고를 원근 합성해 용기에 인쇄된 느낌을 내고, 그 위에 제품명·헤드라인을 얹음. 외부 API 호출 없음. 합성이 실패하면(에셋 로드 실패 등) 톤 기반 그라데이션으로 자동 대체 | 성공 시 `false`, 대체 시 `true` |

`preliminary=true`가 뜨는 경우는 이제 "용기 합성 실패로 그라데이션 대체" 또는 "`card_info` 없음" 뿐입니다. 응답 형식·규격은 두 경우 모두 확정본과 동일합니다.
