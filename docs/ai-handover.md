...
### ⚠️ 상표 도면 이미지 노출 — 보류 요청 (2026-08-10)

**결론부터: KIPRIS 상표 도면을 사용자 화면에 표시하지 않기를 요청합니다.**

배경을 정확히 적습니다. 백엔드에 이미 이미지 제공 기능이 구현되어 있습니다.

- `TrademarkMatchImageService` + `GET /trademark-analyses/{id}/matches/{rank}/image`
- `TrademarkMatchResponse.imageUrl`
- `docs/frontend-trademark-image-api-handoff.md` 로 프론트 인계 완료
- 소유권 검증·경로 이탈 차단까지 갖춰져 있어 **구현 품질에는 문제가 없습니다.**

보류를 요청하는 이유는 구현이 아니라 **데이터 이용 범위**입니다.

- 기준 데이터는 KIPRIS Plus 벌크 데이터이며, 본 문서 §2에 "외부에 재배포하지 않는다"로
  범위를 적어두었습니다. 상표 도면을 서비스 화면에 게시하는 것이 그 범위 안인지
  확인되지 않았습니다.
- 상용 서비스·외부 공개 시점에는 KIPRIS Plus 이용약관 재확인이 필요합니다(§2).

제안하는 절충안:

1. 이미지 API는 **유지하되 기본 비활성**(플래그)으로 두고, 이용 범위가 확인되면 켠다
2. 화면에는 상표명·출원번호·유사도 점수·`note`를 표시한다
3. 원본 확인이 필요한 사용자에게는 KIPRIS 상세 링크를 제공한다
   (출원번호는 공개 정보이므로 링크 제공은 재배포에 해당하지 않는다)

이미지를 감추면 `note`가 사용자에게 제공되는 **유일한 근거**가 되므로, `note` 표시가
함께 반영되어야 합니다.

> 이 항목은 AI 팀 단독 결정 사항이 아닙니다. 기획서 §18과도 충돌하므로
> 백엔드·기획과 합의가 필요합니다.
