...
- `logoStyle`은 현재 `combination`만 허용. `topK`는 1~20 (현재 3).
...
`imagePath`는 `data/trademarks/` 기준 상대경로입니다. 브라우저 노출용 이미지 API는 백엔드가 이 경로를 받아 서빙합니다.
...
```text
similarity = round(clamp(13.7 × z − 7.3, 0, 100))
```

`z`는 쿼리 벡터와 전체 상표 간 코사인 유사도 분포의 표준점수입니다. 등록 상표 간 1위 z 분포(200건 샘플, 중앙값 2.72 / 상위 5% 4.91)에 앵커링해 **이미 공존 등록된 수준 = 30점**, **상위 5% = 60점**이 되도록 매핑했습니다.
...
4. **`note` 필드는 미구현입니다.** Gemini 기반 프로토타입만 검증했고 API 계약에도 포함돼 있지 않습니다.
...
- 분석 결과 MariaDB 저장
- 프론트 결과 화면
- `imagePath`를 브라우저에 서빙하는 이미지 API
- 생성 로고 원본 다운로드
