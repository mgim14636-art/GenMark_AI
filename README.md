...
| 마크인포 등 상표 전문 서비스 | 자체 딥러닝 상표이미지 검색 기술을 보유하지만, 최종 판단은 변리사 검수·보고서를 거치는 유료 컨설팅형 프로세스 | ✅ 사람 개입 없이 즉시·무료로 셀프 스크리닝 — 상담이 필요한 순간을 스스로 먼저 가늠하게 함 |
| 템플릿형 로고 메이커 | 기존 템플릿 조합·커스터마이징 중심 | ✅ 벡터(SVG) 기반 완전 생성 + 브라우저 내 요소별 편집 |

> 마크인포는 회사소개 기준 딥러닝(TensorFlow) 기반 상표이미지 검색 솔루션(TTA 인증, 특허출원 2건)을 자체 개발했지만, 실제 서비스 흐름은 "1:1 상표 매니저 배정 → 변리사 검수 하 권리범위 확인 → 검토 보고서 제공"으로 사람이 최종 판단합니다. GenMark-AI는 그 앞단에서 사람 개입 없이 즉시 점수를 보여주는 셀프서비스형이라는 점이 실질적 차이입니다.
