...
const targetAgeOptions = [
  { label: '10-20대', apiValue: '10~20', description: '트렌드와 개성을 중시하는 고객' },
  { label: '30-40대', apiValue: '30~40', description: '일상과 균형을 중시하는 고객' },
  { label: '50-60대', apiValue: '50~60', description: '편안함과 신뢰를 중시하는 고객' },
  { label: '전 연령층', apiValue: '전 연령층', description: '폭넓은 고객을 위한 브랜드' },
] as const

const targetAgeLabelByApiValue = Object.fromEntries(targetAgeOptions.map((option) => [option.apiValue, option.label])) as Record<string, string>
const toTargetAgeApiValue = (value: string) => targetAgeOptions.find((option) => option.label === value)?.apiValue ?? '전 연령층'
const toTargetAgeLabel = (value: string | null | undefined) => value ? targetAgeLabelByApiValue[value] ?? '전 연령층' : '전 연령층'
          ...
          <section className="brand-details-section target-audience-section" aria-labelledby="target-audience-title">
            ...
            <div className="target-audience-grid" role="group" aria-label="주요 타겟 선택">
              {targetAgeOptions.map((option) => {
                const selected = targetAge === option.label
                return (
                  <button
                    key={option.label}
                    type="button"
